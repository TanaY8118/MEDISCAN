...
public class ReminderController {
    ...
    /**
     * Mobile polling endpoint — called every 15 minutes by the app.
     * Returns due reminders and low-stock alerts for the authenticated user only.
     * Principal is resolved internally by NotificationService; no userId parameter
     * is needed.
     */
    @GetMapping("/due")
    public ResponseEntity<DueRemindersResponse> getDueReminders() {
        List<DueReminderDTO> reminders = notificationService.getDueRemindersForCurrentUser();
        List<StockAlertDTO> alerts = notificationService.getLowStockAlertsForCurrentUser();

        return ResponseEntity.ok(DueRemindersResponse.builder()
                .reminders(reminders)
                .stockAlerts(alerts)
                .build());
    }
