...
public class HistoryService {
    ...
    private TimelineEventDTO mapInventoryLogToTimeline(InventoryLog log) {
        // Fetch inventory entity explicitly since the lazy-loaded relation might be
        // null in the 1st level cache
        com.mediscan.inventory.entity.Inventory inventory = inventoryRepository.findById(log.getInventoryId())
                .orElseThrow(() -> new IllegalStateException("Inventory not found for log: " + log.getId()));

        // Safely get medicine name (handles missing medicine gracefully)
        String medicineName = getMedicineNameSafe(inventory.getMedicineId());
        ...
        metadata.put("unit", inventory.getUnit());
        ...
        return TimelineEventDTO.builder()
                ...
                .medicineName(medicineName)
                .description(getStockDescription(log, inventory.getUnit()))
                .metadata(metadata)
                .build();
    }
     ...
     *         remaining)")
     */
    private String getStockDescription(InventoryLog log, String unit) {
